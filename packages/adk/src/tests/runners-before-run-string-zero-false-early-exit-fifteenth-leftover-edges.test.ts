...
class TruthyBeforeRunPlugin extends BasePlugin {
	constructor(private readonly value: unknown) {
		super("truthy-before-run-fifteenth");
	}
...
/**
 * Fifteenth leftover (HEAVY tip-relaunch residual after tip #269 / 03ff90a8 after providers; supersedes closed #273/#262): twelfth leftover pins classic falsy beforeRun results
 * continuing into the agent. String `"0"` / `"false"` are truthy so
 * `if (earlyExitResult)` early-exits (agent never runs).
 */
describe("runners before-run string-zero/false early-exit fifteenth leftover", () => {
