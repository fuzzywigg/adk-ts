...
describe("InMemoryArtifactService", () => {
	...
	it("resolves a user-scoped ref URI when sessionId is omitted from the URI", async () => {
		const service = new InMemoryArtifactService();
		await service.saveArtifact({
			...base,
			filename: "user:profile.json",
		...
		expect(
			await service.loadArtifact({ ...base, filename: "link.txt" }),
	...
	it("loadArtifact with deeply negative index beyond length returns null", async () => {
		const service = new InMemoryArtifactService();
		await service.saveArtifact({
			...base,
			filename: "neg.txt",
	...
	it("deleteArtifact is a no-op for unknown filenames", async () => {
		const service = new InMemoryArtifactService();
		await expect(
			service.deleteArtifact({ ...base, filename: "ghost.txt" }),
		).resolves.toBeUndefined();
	...
	it("returns null for empty text-only artifact without fileData or inlineData", async () => {
		const service = new InMemoryArtifactService();
		await service.saveArtifact({
			...base,
			filename: "empty-text.txt",
			artifact: { text: "" },
		});
		expect(
			await service.loadArtifact({ ...base, filename: "empty-text.txt" }),
		).toBeNull();
	});
