...
describe("BuiltInPlanner remainder edges (TOKENMAXX)", () => {
	it.each([
	...
	it("repeated apply overwrites thinkingConfig in place", () => {
		const planner = new BuiltInPlanner({
	...
	it("initializes config from nullish via || {}", () => {
		const planner = new BuiltInPlanner({
			thinkingConfig: { includeThoughts: true },
		});
		const request = new LlmRequest();
		request.config = undefined as any;
		planner.applyThinkingConfig(request);
