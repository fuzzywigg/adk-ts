...
class TruthyOnUserMessagePlugin extends BasePlugin {
	constructor(private readonly value: unknown) {
		super("truthy-on-user-twentieth");
	}
...
/**
 * Twentieth leftover: twelfth leftover pins classic falsy onUserMessage
 * keeping the original Content. String `"0"` / `"false"` pass
 * `if (modifiedUserMessage)` and replace userMessage; without artifact blob
 * saves, Runner spreads the string into content (`{..."0"}`) so original text
 * is lost (no TypeError — spread is valid on strings).
 */
describe("runners on-user-message string-zero/false replace twentieth leftover", () => {
