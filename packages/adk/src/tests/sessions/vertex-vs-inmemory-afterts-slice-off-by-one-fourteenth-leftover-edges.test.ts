...
/**
 * Fourteenth leftover: afterTimestamp walk finds last event with
 * timestamp < threshold at index i. In-memory uses slice(i + 1);
 * Vertex uses slice(i) — off-by-one that keeps the boundary predecessor.
 */
describe("vertex vs in-memory afterTimestamp slice off-by-one fourteenth leftover", () => {
	beforeEach(() => {
	...
	it("afterTimestamp:20 on [10,20,30] — in-memory drops 10; Vertex keeps 10", async () => {
