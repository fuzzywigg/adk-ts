...
/**
 * Nineteenth leftover (HEAVY tip-relaunch residual after #253 / tip #254):
 * reversible workflow assertions for the logger/env Vitest slice. Softened
 * while agents true-asymmetry twenty-first owns the focused filter pin — pins
 * lint/build/test only so logger/env nineteenth leftovers stay gated via
 * full `pnpm test`.
 */
describe("logger/env ci workflow nineteenth leftover edges", () => {
	...
	it("push.yml focused gate remains agents twenty-first (logger/env soft)", () => {
		const push = readFileSync(
			resolve(root, ".github/workflows/push.yml"),
			"utf8",
		);
		expect(push).toContain("vitest run twenty-first-leftover-edges");
	});

	it("ci.yml still pins Node 22 matching package engines for env-sensitive Vitest", () => {
		const ci = readFileSync(resolve(root, ".github/workflows/ci.yml"), "utf8");
