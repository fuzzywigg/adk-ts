...
/**
 * Twentieth leftover (HEAVY tip-relaunch residual after providers #269 /
 * tip `03ff90a` / auth-memory #258): soft reversible workflow pins —
 * auth/memory nineteenth still owns focused `push.yml` filter; this
 * twentieth residual deepen stays gated via full `pnpm test`. No
 * workflow retarget (tests-only). Vitest remains on 3.x.
 */
