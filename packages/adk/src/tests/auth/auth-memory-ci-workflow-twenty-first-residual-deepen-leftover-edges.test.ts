...
/**
 * Twenty-first leftover residual deepen (soft after tip #292 / #287):
 * auth/memory string-Infinity / Object(1) / Object(false) residual deepen
 * stays gated via full pnpm test. Soft pin only — does not retarget
 * push.yml focused `twentieth-leftover-edges` gate. Vitest stays on 3.x.
 */
describe("auth/memory ci workflow twenty-first residual deepen leftover edges", () => {
	...
	it("push.yml focused gate still pins twentieth leftover edges (no retarget)", () => {
		const push = readFileSync(
			resolve(root, ".github/workflows/push.yml"),
			"utf8",
		);
