 ...
 * pins Infinity enter; nineteenth pins numeric-string `"60"`. SameValueZero
 * residual: `-0` is falsy (`!!(-0)` false) even though `Object.is(-0, 0)` is
 * false → expiresAt unset (skip arm), not `(-0)*1000` enter.
 */
describe("oauth2 expiresIn negzero falsy-skip twentieth leftover", () => {
