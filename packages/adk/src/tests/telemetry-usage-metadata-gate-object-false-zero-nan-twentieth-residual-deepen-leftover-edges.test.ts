 ...
 * `if (usageMetadata)` — boxed falsy `Object(false)` / `Object(0)` /
 * `Object(NaN)` enter; missing token props coalesce via `|| 0`.
 */
describe("telemetry usageMetadata gate object-false/zero/nan twentieth residual deepen", () => {
	const invocation = {
		invocationId: "inv",
		userId: "u",
		session: { id: "s" },
	...
	])("boxed residual usageMetadata ($label) enters token || 0 arm", async ({
		value,
	}) => {
