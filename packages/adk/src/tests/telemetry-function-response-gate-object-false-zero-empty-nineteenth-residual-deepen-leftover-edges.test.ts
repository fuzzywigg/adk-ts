 ...
 * `if (functionResponse)` — boxed-falsy / `"-Infinity"` / `-1` enter
 * and hit `id || "<not specified>"` (no id prop → sentinel).
 */
describe("telemetry functionResponse gate object-false/zero/empty nineteenth residual deepen", () => {
	...
	])("truthy residual functionResponse ($label) enters id || sentinel", async ({
		value,
	}) => {
