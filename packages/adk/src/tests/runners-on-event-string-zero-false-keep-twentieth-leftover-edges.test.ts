...
/**
 * Twentieth leftover: `yield modifiedEvent || event` — seventh leftover pins
 * classic falsy (`null`/`0`/`""`/`false`) falling back to the original event.
 * String `"0"` / `"false"` are truthy so the coalesced yield keeps them.
 */
describe("runners onEvent string-zero/false keep twentieth leftover", () => {
