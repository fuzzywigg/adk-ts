...
const mockContext: InvocationContext = {
	invocationId: "twenty-first-loop-esc-resid",
	agent: {} as any,
	branch: "",
	session: {
		id: "ses-loop-21r",
 ...
 * `if (escalate)` — POSITIVE_INFINITY / `1` / `{}` / `Object(true)` /
 * `"Infinity"` stop; `NaN` continues under maxIterations.
 */
describe("LoopAgent escalate posinf/nan/object-true twenty-first residual deepen", () => {
	...
	it.each([
		...
		const sub = new MockSubAgent("resid_esc");
		let calls = 0;
