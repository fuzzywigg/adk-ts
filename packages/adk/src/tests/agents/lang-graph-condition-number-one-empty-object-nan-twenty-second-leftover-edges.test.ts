 ...
 * number `1` / `{}` execute target; `NaN` skips.
 */
describe("LangGraphAgent condition number-one/empty-object/NaN twenty-second leftover", () => {
	...
	it.each([
		...
		const root = new MockAgent("one_root");
		const child = new MockAgent("one_child");
		const graph = new LangGraphAgent({
			name: "one_cond",
			description: "d",
			nodes: [
				{ name: "one_root", agent: root, targets: ["one_child"] },
				{
					name: "one_child",
					agent: child,
					targets: [],
