...
/**
 * Twentieth leftover: eighteenth pins callback-result `"0"`/`"false"` truthy.
 * Residual `"true"` is also truthy Content → endInvocation short-circuit.
 */
describe("BaseAgent callback-result string-true truthy twentieth leftover", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	});

	it('before callback returning "true" short-circuits as truthy content', async () => {
		const later = vi.fn(() => ({ parts: [{ text: "never" }] }));
		const agent = new TestAgent({
			name: "before_str_true",
			beforeAgentCallback: [() => "true" as any, later],
		});
		const { events, childCtx } = await collect(agent);
		expect(later).not.toHaveBeenCalled();
		expect(agent.runAsyncImplMock).not.toHaveBeenCalled();
		expect(events).toHaveLength(1);
		expect(events[0].content).toBe("true");
