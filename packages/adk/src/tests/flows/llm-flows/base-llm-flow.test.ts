...
describe("BaseLlmFlow leftover callback/toolsDict/parts pads", () => {
	it("_postprocessAsync defaults toolsDict to {} when undefined on the request", async () => {
		const flow = new InspectableFlow();
		flow.responseProcessors = [];
		const llmRequest = new LlmRequest();
		delete (llmRequest as any).toolsDict;
	...
	it("_handleAfterModelCallback continues past falsy callbacks then returns truthy", async () => {
		const flow = new InspectableFlow();
		const modelEvent = new Event({ id: "me", author: "agent" });
		const agent = {
			name: "after-cb",
			canonicalAfterModelCallbacks: [
				...
				() =>
					({
						content: { role: "model", parts: [{ text: "after-hit" }] },
					}) as LlmResponse,
			],
		};
		...
		const result = await flow._handleAfterModelCallback(
			makeCtx({ agent }),
			{ content: { role: "model", parts: [{ text: "raw" }] } } as LlmResponse,
			modelEvent,
		);
		expect(result?.content?.parts?.[0]).toEqual({ text: "after-hit" });
	});

	it("_handleBeforeModelCallback returns undefined when callback list is empty", async () => {
		const flow = new InspectableFlow();
		const result = await flow._handleBeforeModelCallback(
			makeCtx({
				agent: {
			...
			new LlmRequest(),
			new Event({ id: "me", author: "empty-before" }),
		);
		expect(result).toBeUndefined();
	});

	it("_handleAfterModelCallback returns undefined when callback list is empty", async () => {
		const flow = new InspectableFlow();
		const result = await flow._handleAfterModelCallback(
			makeCtx({
				agent: {
			...
			new Event({ id: "me", author: "empty-after" }),
		);
		expect(result).toBeUndefined();
	});

	it("_finalizeModelResponseEvent tolerates response content without parts", () => {
		const flow = new InspectableFlow();
		getLongRunningFunctionCallsMock.mockReturnValue(new Set());
		const event = flow._finalizeModelResponseEvent(
			new LlmRequest({ toolsDict: { t: {} as any } }),
			{ content: { role: "model" }, finishReason: "STOP" } as LlmResponse,
			new Event({ id: "me", author: "agent" }),
		);
		expect(event.content?.role).toBe("model");
		expect(event.content?.parts).toBeUndefined();
		expect(event.getFunctionCalls()).toEqual([]);
	});

	it("_callLlmAsync applies after-model pad when response has no parts", async () => {
		const flow = new InspectableFlow();
		flow.requestProcessors = [];
		flow.responseProcessors = [];
		const agent = {
			...
			canonicalAfterModelCallbacks: [
				...
				() =>
					({
						content: { role: "model", parts: [{ text: "padded" }] },
					}) as LlmResponse,
			],
			canonicalModel: {
				model: "fake",
		...
		const responses = await collect(
			flow._callLlmAsync(
				makeCtx({ agent }),
				new LlmRequest(),
				new Event({ id: "me", author: "no-parts-after" }),
			),
		);
		expect(responses).toHaveLength(1);
		expect((responses[0] as LlmResponse).content?.parts?.[0]).toEqual({
	...
	it("_postprocessLive passes empty toolsDict when llmRequest.toolsDict is undefined", async () => {
		const flow = new InspectableFlow();
		flow.responseProcessors = [];
		const llmRequest = new LlmRequest();
		delete (llmRequest as any).toolsDict;
		...
		await collect(
			flow._postprocessLive(
				...
				llmRequest,
				{
					content: {
						role: "model",
						parts: [{ functionCall: { id: "live-1", name: "t", args: {} } }],
					},
					turnComplete: true,
				} as LlmResponse,
				new Event({ id: "me", author: "agent" }),
	...
	it("_handleBeforeModelCallback returns undefined when canonical hooks property is missing", async () => {
		const flow = new InspectableFlow();
		const result = await flow._handleBeforeModelCallback(
			makeCtx({ agent: { name: "bare" } }),
			new LlmRequest(),
			new Event({ id: "me", author: "bare" }),
		);
		expect(result).toBeUndefined();
	});

	it("_handleAfterModelCallback returns undefined when canonical hooks property is missing", async () => {
		const flow = new InspectableFlow();
		const result = await flow._handleAfterModelCallback(
			makeCtx({ agent: { name: "bare" } }),
			{ content: { parts: [{ text: "x" }] } } as LlmResponse,
