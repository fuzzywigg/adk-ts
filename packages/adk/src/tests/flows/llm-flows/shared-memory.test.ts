...
describe("sharedMemoryRequestProcessor contents/parts leftover edges", () => {
	it("treats session contents without parts as empty strings during dedupe", async () => {
		const searchMemory = vi.fn(async () => ({
			memories: [
				{
					author: "mem",
					content: { role: "user", parts: [{ text: "unique-memory" }] },
				},
			],
		}));
		const { context, llmRequest } = makeContext({
		...
		await drain(sharedMemoryRequestProcessor.runAsync(context, llmRequest));
		expect(llmRequest.contents?.at(-1)?.parts?.[0]?.text).toBe(
			"[mem] said: unique-memory",
		);
	});

	it("coalesces non-text user parts to empty strings in the memory query", async () => {
		const searchMemory = vi.fn(async () => ({ memories: [] }));
		const { context, llmRequest } = makeContext({
			memoryService: { searchMemory } as any,
			events: [
		...
		await drain(sharedMemoryRequestProcessor.runAsync(context, llmRequest));
		expect(searchMemory).toHaveBeenCalledWith(
			expect.objectContaining({ query: " caption " }),
		);
	});

	it("coalesces non-text memory parts to empty strings when injecting", async () => {
		const searchMemory = vi.fn(async () => ({
			memories: [
				{
					author: "blob",
	...
	it("skips memory injection when memory text already appears in a parts-less content slot", async () => {
		const searchMemory = vi.fn(async () => ({
			memories: [
				{
					author: "past",
		...
		const beforeLen = llmRequest.contents?.length ?? 0;
		await drain(sharedMemoryRequestProcessor.runAsync(context, llmRequest));
		expect(llmRequest.contents?.length).toBe(beforeLen + 1);
		expect(llmRequest.contents?.at(-1)?.parts?.[0]?.text).toBe("[past] said: ");
	});

	it("initializes undefined contents then injects memory with empty author label", async () => {
		const searchMemory = vi.fn(async () => ({
			memories: [
				{
