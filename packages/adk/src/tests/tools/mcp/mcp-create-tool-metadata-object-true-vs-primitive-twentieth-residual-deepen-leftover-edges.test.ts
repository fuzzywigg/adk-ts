...
/**
 * Twentieth leftover residual deepen (complements fourteenth array metadata):
 * `"metadata" in … && typeof metadata === "object"` — `Object(true)` /
 * `Object(1)` are objects so they win over `_meta` (flags default via ??);
 * primitive `true` / `1` are non-objects and fall through to `_meta`.
 */
