...
/**
 * Twentieth leftover (HEAVY tip-relaunch residual complement after tip #254 /
 * open #259): reversible workflow assertions for the tools/mcp complement
 * Vitest slice (PATH/env/maxTokens/messages/filter/schema). Softened while
 * agents true-asymmetry twenty-first owns the focused filter pin — pins
 * lint/build/test only (no push.yml retarget; avoids conflicting with #259).
 */
