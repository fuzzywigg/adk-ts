...
/**
 * Twenty-first leftover: reversible workflow assertions for the tools/mcp
 * boolean-true / string-true / SameValueZero `-0` / empty-array /
 * NEGATIVE_INFINITY residual Vitest slice after tip #269 / #259. Softened
 * while tip auth/memory owns the focused `nineteenth-leftover-edges` filter
 * pin — pins lint/build/test only (no push.yml retarget).
 */
describe("tools/mcp residual ci workflow twenty-first leftover edges", () => {
