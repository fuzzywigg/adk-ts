...
/**
 * Seventeenth leftover: reversible workflow assertions for the tools/MCP
 * session-lifecycle Vitest slice. Pins focused seventeenth leftover gate on
...
describe("tools/mcp session lifecycle ci workflow seventeenth leftover edges", () => {
	...
	it("push.yml runs focused seventeenth leftover slice then pnpm test", () => {
		...
		expect(push).toContain("seventeenth-leftover-edges");
