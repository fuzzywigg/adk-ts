...
describe("createTool matrix leftover edges", () => {
	...
	it("preserves array and nested object truthy results", async () => {
		const tool = createTool({
			name: "complex_result",
	...
	it("applies zod defaults when args omit fields", async () => {
		const tool = createTool({
			name: "defaults_tool",
	...
	it("safeExecute validates required fields through BaseTool", async () => {
		const tool = createTool({
			name: "safe_create",
