...
/**
 * Seventeenth leftover: Anthropic `max_tokens: maxOutputTokens || 1024` vs
 * OpenAI / AI SDK pass-through (no default). Fourteenth covered Anthropic-only;
 * OpenAI sixth only forwarded literal `0`.
 */
describe("providers max-tokens or-default vs passthrough seventeenth leftover edges", () => {
	...
	])("Anthropic defaults falsy $label to 1024; OpenAI/AI SDK pass through", async ({
		value,
	}) => {
	...
	it("undefined maxOutputTokens → Anthropic 1024; OpenAI/AI SDK undefined", async () => {
		await drainAnthropic(undefined);
		expect(anthropicCreate).toHaveBeenCalledWith(
			expect.objectContaining({ max_tokens: 1024 }),
		);

		await drainOpenAi(undefined);
		expect(openaiCreate).toHaveBeenCalledWith(
			expect.objectContaining({ max_tokens: undefined }),
		);

		await drainAiSdk(undefined);
		expect(generateText).toHaveBeenCalledWith(
