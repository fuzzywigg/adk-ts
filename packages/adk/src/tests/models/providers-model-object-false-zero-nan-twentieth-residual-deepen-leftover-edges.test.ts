 ...
 * `llmRequest.model || this.model` — boxed falsy `Object(false)` / `Object(0)` /
 * `Object(NaN)` keep (bypass constructor fallback).
 */
describe("providers model object-false/zero/nan twentieth residual deepen", () => {
	...
	it.each([
		{ label: "Object(false)", model: Object(false) },
		{ label: "Object(0)", model: Object(0) },
		{ label: "Object(NaN)", model: Object(Number.NaN) },
	...
	it("Google convert path keeps residual model Object(0) (identity via ||)", () => {
		const boxed = Object(0);
		const req = new LlmRequest({
			model: boxed as any,
			contents: [{ role: "user", parts: [{ text: "hi" }] }],
		});
		expect(req.model || (google as any).model).toBe(boxed);
		expect(req.model || (openai as any).model).toBe(boxed);
		expect(req.model || (anthropic as any).model).toBe(boxed);
		expect(req.model || (aiSdk as any).model).toBe(boxed);
	});
});
