 ...
 * `llmRequest.model || this.model` — boxed-falsy / `"-Infinity"` / `-1`
 * keep (bypass constructor fallback).
 */
describe("providers model object-false/zero/empty nineteenth residual deepen", () => {
	...
	it.each([
		...
		{ label: 'string "-Infinity"', model: "-Infinity" },
		{ label: "number -1", model: -1 },
	])("Google/AI SDK || identity keeps residual model $label", ({ model }) => {
		const req = new LlmRequest({
			model: model as any,
			contents: [{ role: "user", parts: [{ text: "hi" }] }],
		});
		expect(req.model || (google as any).model).toBe(model);
		expect(req.model || (openai as any).model).toBe(model);
		expect(req.model || (anthropic as any).model).toBe(model);
		expect(req.model || (aiSdk as any).model).toBe(model);
	});
});
